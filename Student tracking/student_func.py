...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_student (col_fname,col_lname,col_fullname,col_phone,col_email,col_currentCourse) \
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email,col_currentCourse FROM tbl_student WHERE col_fullname = (?)""", [value])
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # remove blank spaces
    var_lname = var_lname.strip() 
    var_fname = var_fname.title()# capitalize first letter 
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,var_lname)) # combine to make fullname
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0) and(len(var_currentCourse) > 0): # makes them provide both names
        conn = sqlite3.connect('db_student.db')
        with conn:
            cursor = conn.cursor()
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_student (col_fname,col_lname,col_fullname,col_phone,col_email,col_currentCourse) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email,var_currentCourse))
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) # delete whats in the box
                conn.commit()
        else:
...
def onUpdate(self):
    try:
        ...
        return
    # The user will only be alowed to update changes for phone and emails and course.
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_currentCourse) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            cur.execute("""SELECT COUNT(col_currentCourse) FROM tbl_student WHERE col_currentCourse = '{}'""".format(var_currentCourse))
            count4 = cur.fetchone()[0]
           
            if count == 0 or count2 == 0 or count4 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_student SET col_phone = '{0}',col_email = '{1}',col_currentCourse = '{2}' WHERE col_fullname = '{3}'""".format(var_phone,var_email,var_currentCourse,var_value))
                        onClear(self)
                        conn.commit()
                else:
                    messagebox.showinfo("Cancel request","No changes have been made to ({}).".format(var_value))
            else:
                messagebox.showinfo("No changes detected","all ({}) and ({}) and ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(var_phone, var_email, var_currentCourse))
            onClear(self)
        conn.close()
    else:
