...
def load_gui(self):
    # creates the tkinter widgets and position and configuration
    ...
    self.lbl_user.grid(row=8,column=0,padx=(27,0),pady=(10,0),sticky=N+W)
    ...
    # defines the listbox with a scrollbar and grid them
    self.scrollbar1 = Scrollbar(self.master,orient=VERTICAL)
    self.lstList1 = Listbox(self.master,exportselection=0,yscrollcommand=self.scrollbar1.set)
    self.lstList1.bind('<<ListboxSelect>>',lambda event: student_func.onSelect(self,event))
    self.scrollbar1.config(command=self.lstList1.yview)
    self.scrollbar1.grid(row=1,column=7,rowspan=9,columnspan=1,padx=(0,0),pady=(0,0),sticky=N+E+S)
